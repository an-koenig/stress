...
stress3 <- stress2 %>% select(Country, Dem_gender, Dem_age, AD_gain, PSS10_avg, neu, ext, ope, agr, con) 
...
stress5$gain_safe <- 99

for (i in 1:nrow(stress5)){
  if (grepl("Program A", stress5$AD_gain[i])){
    stress5$gain_safe[i] <- 1}
  if (grepl("Program B", stress5$AD_gain[i])){
    stress5$gain_safe[i] <- 0}
}

stress5$AD_gain <- NULL
...
#find equivalence class of DAG

ec1 <- bnlearn::cpdag(Res, wlbl = TRUE) #keeping blacklisted arcs
ec2 <- bnlearn::cpdag(Res, wlbl = FALSE) #not keeping blacklisted arcs
...
g <- qgraph(ec1, nodeNames = Labels, legend.cex = 0.4,
            asize = 5, edge.color = "black") 

g <- qgraph(ec2, nodeNames = Labels, legend.cex = 0.4,
            asize = 5, edge.color = "black") 
